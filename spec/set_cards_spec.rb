...
describe SetCards do
  ...
  subject(:set_cards) { described_class.new }

  it 'generates all possible combinations of 3 cards' do
    expect(set_cards.all_triples.length).to eq 85320
  end
  ...
  it 'does not validate a combination that has two cards with the same attribute' do
    ...
    expect(set_cards.valid_set?(combination)).to be_falsey
  end
  ...
  it 'validates a combination of all distinct cards' do
    ...
    expect(set_cards.valid_set?(combination)).to be_truthy
  end
  ...
  it 'finds all valid sets' do
    expect(set_cards.count_valid_sets(subject.all_triples)).to eq 1080
  end
end
